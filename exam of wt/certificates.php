...
<head>
  ...
  <title>insert certificates</title>
...
<center>
  <h5>CERTIFICATES INFORMATION</h5>
  <form action="certificates.php" method="POST" onsubmit="return confirmInsert();" style="background-color: white; align-items: center; width: 300px; height: 300px; margin-top: 20px;"><br><br>
    ...
    <label>Enter IssueDate:</label><br>
    <input type="Date" name="IssueDate" required><br>
    <label>Enter ExpiryDate:</label><br>
    <input type="Date" name="ExpiryDate" required><br>
    <label>Enter CertificateURL:</label><br>
    <input type="text" name="CertificateURL" required><br>
...
if ($_SERVER["REQUEST_METHOD"] =="POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO certificates ( ParticipantID, WorkshopID ,IssueDate ,ExpiryDate, CertificateURL) VALUES (?, ?, ?, ?,?)");
    $stmt->bind_param("sssss",  $ParticipantID, $WorkshopID, $IssueDate, $ExpiryDate, $CertificateURL);
...
<body>
    <center><h2>TABLE OF CERTIFICATES DATA</h2></center>
    ...
    <table id="dataTable">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr><td>" .  $row["CertificateID"] .
                     "</td><td>" . $row["ParticipantID"] .
                     "</td><td>" . $row["WorkshopID"] .
                     "</td><td>" . $row["IssueDate"] .
                     "</td><td>" . $row["ExpiryDate"] .
                     "</td><td>" . $row["CertificateURL"] .
                     "</td><td><a href='updatecertificates.php?CertificateID=$CertificateID'>Update</a>
                     </td><td><a href='deletecertificates.php?CertificateID=$CertificateID'>Delete</a></td>
                     </tr>";        
            }
        } else {
