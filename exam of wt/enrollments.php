...
<head>
  ...
  <title>insert enrollments</title>
...
<center>
  <h5>ENROLLEMENTS INFORMATION</h5>
  <form action="enrollments.php" method="POST" onsubmit="return confirmInsert();" style="background-color: white; align-items: center; width: 300px; height: 250px; margin-top: 20px;">
    ...
    <label>Enter EnrollmentDate:</label><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO enrollments (WorkshopID , ParticipantID, EnrollmentDate) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $WorkshopID,$ParticipantID,$EnrollmentDate);
...
<body>
    <center><h2>TABLE OF ENROLLEMENTS DATA</h2></center>
    ...
    <table id="dataTable">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr><td>" . $row["EnrollmentID"] .
                     "</td><td>" . $row["WorkshopID"] .
                     "</td><td>" . $row["ParticipantID"] .
                     "</td><td>" . $row["EnrollmentDate"] .
                     "</td><td><a href='updateenrollements.php?EnrollmentID=$EnrollmentID'>Update</a>
                     </td><td><a href='deleteenrollements.php?EnrollmentID=$EnrollmentID'>Delete</a></td>
                     </tr>";     
            }
        } else {
