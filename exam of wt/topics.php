...
<center>
  ...
  <form action="topics.php" method="POST" onsubmit="return confirmInsert();" style="background-color: white; align-items: center; width: 300px; height: 150px; margin-top: 20px;">
    
    <label>Enter TopicName:</label><br>
    <input type="text" name="TopicName" required><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO topics ( TopicName ) VALUES (?)");
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of topics</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>TABLE OF TOPICS DATA</h2></center>
    ...
    <table id="dataTable">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $TopicID = $row['TopicID'];
                echo "<tr><td>" . $row["TopicID"] .
                     "</td><td>" . $row["TopicName"] .
                     "</td><td><a href='updatetopics.php?TopicID=$TopicID'>Update</a>
                     </td><td><a href='deletetopics.php?TopicID=$TopicID'>Delete</a></td>
                     </tr>";        
            }
        } else {
