...
<center>
  ...
  <form action="participants.php" method="POST" onsubmit="return confirmInsert();" style="background-color: white; align-items: center; width: 300px; height: 250px; margin-top: 20px;">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO participants ( Name, Email, Phone) VALUES (?, ?, ?)");
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of participants</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>TABLE OF PARTICIPANTS DATA</h2></center>
    ...
    <table id="dataTable">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr><td>" . $row["ParticipantID"] .
                     "</td><td>" . $row["Name"] .
                     "</td><td>" . $row["Email"] .
                     "</td><td>" . $row["Phone"] .
                     "</td><td><a href='updateparticipants.php?ParticipantID=$ParticipantID'>Update</a>
                     </td><td><a href='deleteparticipants.php?ParticipantID=$ParticipantID'>Delete</a></td>
                     </tr>";             
            }
        } else {
